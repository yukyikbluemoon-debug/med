// ✅ app.js - เวอร์ชันทำงานได้จริง (แทนที่ทั้งไฟล์)
const PERIODS = [
...
function buildAppHTML() {
  return `
    <main class="app-shell">
      <header class="topbar">
        <div><p class="eyeline">วันนี้</p><h1>ตารางยา</h1></div>
        <button class="icon-button" type="button" aria-label="ตั้งค่า" data-action="open-config">${icon("settings")}</button>
      </header>
      <section class="summary-band">
        <div><span>ช่วงตอนนี้</span><strong>${getCurrentPeriodLabel()}</strong></div>
  ...
  return `<div class="schedule-list">${PERIODS.map((period) => {
    ...
    return `
      <section class="period-section">
        <div class="period-heading"><h2>${period.label}</h2><time>${state.config.times?.[period.id] || period.defaultTime}</time></div>
        ${items.length ? items.map((m) => renderMedicineRow(period.id, m)).join("") : `<div class="empty-row">ยังไม่มียาสำหรับช่วงนี้</div>`}
      </section>`;
  }).join("")}</div>`;
}
  ...
  return `
    <article class="medicine-row">
      ...
      <div class="row-actions">
        <button type="button" aria-label="แก้ไข" data-action="edit-medicine" data-id="${medicine.id}">${icon("edit")}</button>
        <button type="button" aria-label="ลบ" data-action="delete-medicine" data-id="${medicine.id}">${icon("trash")}</button>
      </div>
    </article>`;
}
...
function renderConfig() {
  return `
    <section class="config-panel">
      ...
      <div class="time-settings">${PERIODS.map((p) => `
        <div class="time-row">
          <label><span>${p.label}</span><input type="time" value="${state.config.times?.[p.id] || p.defaultTime}" data-time="${p.id}" /></label>
          <label class="switch"><input type="checkbox" ${state.config.notify?.[p.id] ? "checked" : ""} data-notify="${p.id}" /><span></span></label>
        </div>`).join("")}</div>
  ...
  return `
    <div class="sheet-backdrop" role="dialog" aria-modal="true">
      <form class="medicine-sheet" id="medicine-form">
        ...
        <div class="sheet-title"><h2>${state.editingMedicine ? "แก้ไขยา" : "เพิ่มยา"}</h2><button type="button" class="icon-button" data-action="close-form">${icon("close")}</button></div>
        <input type="hidden" name="id" value="${escapeAttribute(m.id || "")}" />
        <input type="hidden" name="imageUrl" value="${escapeAttribute(m.imageUrl || "")}" />
        <input type="hidden" name="imageFileId" value="${escapeAttribute(m.imageFileId || "")}" />
        <label class="field"><span>ชื่อยา</span><input name="name" value="${escapeAttribute(m.name)}" placeholder="เช่น ยาลดความดัน" /></label>
        <label class="field"><span>จำนวนที่กิน</span><input name="dose" value="${escapeAttribute(m.dose)}" placeholder="เช่น 1 เม็ด" /></label>
        <fieldset class="choice-group"><legend>ช่วงเวลา</legend><div class="choice-grid">${PERIODS.map((p) => `
          <label class="choice ${m.periods?.includes(p.id) ? "checked" : ""}"><input type="checkbox" name="periods" value="${p.id}" ${m.periods?.includes(p.id) ? "checked" : ""} />${p.label}</label>`).join("")}</div></fieldset>
        <fieldset class="choice-group"><legend>อาหาร</legend><div class="choice-grid three">${MEAL_OPTIONS.map((meal) => `
          <label class="choice ${m.meal === meal ? "checked" : ""}"><input type="radio" name="meal" value="${meal}" ${m.meal === meal ? "checked" : ""} />${meal}</label>`).join("")}</div></fieldset>
        <section class="photo-panel">
          <div><span>รูปยา</span><p>ถ่ายใหม่หรือเลือกรูปจากเครื่องได้ รูปจะถูกย่อให้อัตโนมัติ</p></div>
          <div id="photo-preview">${renderMedicineImage(m.imageUrl, m.name || "ยา", true)}</div>
        ...
        <label class="field"><span>หมายเหตุ</span><textarea name="note" placeholder="เช่น ดื่มน้ำตาม">${escapeHtml(m.note || "")}</textarea></label>
...
// ✅ Image compression (เวอร์ชันเดียว ไม่ซ้อน)
...
function getCurrentPeriodLabel() {
  ...
  if (h < 11) return "เช้า"; if (h < 16) return "กลางวัน"; if (h < 21) return "เย็น"; return "ก่อนนอน";
}
